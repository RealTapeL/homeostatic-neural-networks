...
def load_data():
    ...
    data = pd.read_csv('mnist_train.csv', header=None, skiprows=1).to_numpy()
    data = data.astype(np.float32)
    #labels = data[:, 0]
    labels = data[:, 0].astype(np.int8)
    num_train = labels.shape[0]  # Get the number of training samples
    y = np.zeros((10, num_train))
    for i in range(num_train):
       y[int(labels[i]), i] = 1  # 在索引前强制转换类型

    #    y[labels[i], i] = 1

    images = data[:, 1:].T / 255.0

    val = pd.read_csv('mnist_test.csv', header=None, skiprows=1).to_numpy()
    vallabels = val[:, 0]
    num_val = vallabels.shape[0]  # Get the number of validation samples
    valy = np.zeros((10, num_val))
    for i in range(num_val):
        valy[int(vallabels[i]), i] = 1
